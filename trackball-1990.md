---
author: "Panagiotis Milopoulos 1126464"
title: "Trackball (1990)"
image_url: "/images/trackball-1990.jpg"
caption: >
 Τα trackballs αποτέλεσαν εναλλακτικό τρόπο ελέγχου του δείκτη, προσφέροντας εργονομία και ακρίβεια.
  Η «αναποδογυρισμένη» λογική του ποντικιού , δηλαδή μια σταθερή συσκευή ως ποντίκι, με κινούμενη σφαίρα
  εμφανίστηκε για να μειώνει την καταπόνηση του καρπού και να λειτουργεί σε περιορισμένο χώρο.
  Με την περιστροφή της σφαίρας, ο χρήστης πετυχαίνει ακριβή πλοήγηση όσο η υπόλοιπη συσκευή παραμένει ακίνητη.
license_url: "https://commons.wikimedia.org/wiki/File:VW_Trackball-Kensington-ExpertMouse5.jpg"
categories:
  - Interaction
  - Input devices
  - Συσκευές εισόδου
  - Ergonomics
tags:
  - Τrackball
  - Κensington
  - Μouse
  - Εrgonomics
  - Hci
---
